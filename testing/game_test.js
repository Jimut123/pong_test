...
async function init(){
    ...
    model =  await tf.loadModel('http://localhost/tfjsmodel1/model.json');
...
// moves paddle by x and y pixels (x is always 0 now)
    ...
    // but only for 2+ frame of a game (we need data from previous frame as well)
     if(this.previous_xs != null){
        ...
        // also use ionly ai's paddle positions
         data_xs = [
            width - this.previous_xs[1], width - this.previous_xs[2], width - this.previous_xs[3],
            width - this.previous_xs[5], width - this.previous_xs[6], width - this.previous_xs[7]
         ];
        // predict move
